// api/push-send.js
// Force Node.js runtime (nécessaire pour web-push)
...
module.exports = async function handler(req, res) {
  ...
  if (isTest) {
    ...
    if (subErr || !sub) return res.status(404).json({ error: 'No subscription found. Make sure notifications are enabled.' });

    try {
      const subscription = JSON.parse(sub.subscription);
      await webpush.sendNotification(subscription, JSON.stringify({
        title: '🧪 Test InvestIQ',
        body: 'Les notifications fonctionnent ! Briefing chaque matin à 8h.',
  ...
  // Mode normal — envoie à tous les abonnés
  ...
  for (const sub of subs) {
    try {
      ...
      if (alerts.length) { title = `⚠️ ${alerts.length} alerte${alerts.length>1?'s':''}`; body = `${alerts[0].name} en baisse · ${body}`; }

      await webpush.sendNotification(subscription, JSON.stringify({ title, body, icon:'/icons/icon-192.png', url:'/', tag:'investiq-daily' }));
      sent++;
    } catch (err) {
      failed++;
