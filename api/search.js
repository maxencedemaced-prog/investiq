export default async function handler(req, res) {
  ...
  try {
    const url = `https://query1.finance.yahoo.com/v1/finance/search?q=${encodeURIComponent(q)}&quotesCount=8&newsCount=0&enableFuzzyQuery=true&lang=fr-FR`;
    ...
    const results = quotes
      ...
      .slice(0, 8)
      .map(q => ({
        ticker: q.symbol,           // ← TOUJOURS le symbole boursier (ex: STM.PA)
    ...
    // Fallback local — entreprises populaires avec vrais tickers
    const popular = [
      ...
      { ticker: 'STM.PA',  name: 'STMicroelectronics',           exchange: 'Euronext Paris' },
      { ticker: 'SOI.PA',  name: 'Soitec',                       exchange: 'Euronext Paris' },
      { ticker: 'RMS.PA',  name: 'Hermès International',         exchange: 'Euronext Paris' },
      { ticker: 'CAP.PA',  name: 'Capgemini',                    exchange: 'Euronext Paris' },
      { ticker: 'DSY.PA',  name: 'Dassault Systèmes',            exchange: 'Euronext Paris' },
      { ticker: 'SAF.PA',  name: 'Safran',                       exchange: 'Euronext Paris' },
