...
function updateDCA() {
  ...
  if (canvas) {
    ...
    const ctx = canvas.getContext('2d');
    const grad =     const grad = ctx.createLinearGradient(0, 0, 0, 240);
...
// updateDCADonut est appelé directement depuis nav dcadcaChartInstance = new Chart(ctx, {
      type: 'line',
      data: {
        labels,
        datasets: [
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            bodyColor: '#fff',
            callbacks: { label: c => ' ' + c.dataset.label + ' : ' + fmtK(c.raw) }
          }
        },
        scales: {
          x: {
            ...
            ticks: { font: { size: 11, weight: '600' }, color: 'rgba(255,255,255,0.35)', maxRotation: 0 }
          },
          y: {
            grid: { color: 'rgba(255,255,255,0.05)' },
            border: { display: false },
            ticks: { font: { size: 11, weight: '600' }, color: 'rgba(255,255,255,0.35)', callback: v => fmtK(v), maxTicksLimit: 6 }
          }
        },
        interaction: { mode: 'index', intersect: false }
      }
