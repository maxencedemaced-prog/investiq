  ...
  if (tableEl) {
    ...
    for (const yr of uniqueMilestones) {
      ...
      rows += `<tr style="border-bottom:1px solid rgba(255,255,255,0.06)">
        <td style="padding:13px 12px;font-weight:700;color:var(--color-text,#e6edf3);font-size:13px">Année ${yr}</td>
        <td style="padding:13px 12px;font-weight:800;color:#4ade80;font-size:14px;letter-spacing:-0.02em">${fmtK(Math.round(fv))}</td>
        <td style="padding:13px 12px;color:rgba(255,255,255,0.45);font-size:13px">${fmtK(Math.round(inv))}</td>
        <td style="padding:13px 12px;color:#4ade80;font-weight:600;font-size:13px">+${fmtK(Math.round(g))}</td>
        <td style="padding:13px 12px">
          <span style="background:rgba(74,222,128,0.12);color:#4ade80;font-weight:700;padding:4px 10px;border-radius:8px;font-size:12px">+${pct}%</span>
        </td>
      </tr>`;
    }
      ...
      <table style="width:100%;border-collapse:collapse">
        <thead>
          <tr style="border-bottom:2px solid #f0f0f0">
            <th style="text-align:left;padding:8px 12px;font-size:10px;color:rgba(255,255,255,0.3);font-weight:700;text-transform:uppercase;letter-spacing:0.08em">ANNÉE</th>
            <th style="text-align:left;padding:8px 12px;font-size:10px;color:rgba(255,255,255,0.3);font-weight:700;text-transform:uppercase;letter-spacing:0.08em">CAPITAL PROJETÉ</th>
            <th style="text-align:left;padding:8px 12px;font-size:10px;color:rgba(255,255,255,0.3);font-weight:700;text-transform:uppercase;letter-spacing:0.08em">INVESTI</th>
            <th style="text-align:left;padding:8px 12px;font-size:10px;color:rgba(255,255,255,0.3);font-weight:700;text-transform:uppercase;letter-spacing:0.08em">GAINS</th>
            <th style="text-align:left;padding:8px 12px;font-size:10px;color:rgba(255,255,255,0.3);font-weight:700;text-transform:uppercase;letter-spacing:0.08em">PERF. ANNUELLE</th>
          </tr>
        </thead>
        <tbody>${trHtml}</tbody>
      </table>
...
function updateDCADonut() {
  ...
  if(get('dca-hero-total')) get('dca-hero-total').textContent = fmtK(Math.round(total));
  if(get('dca-hero-gain'))  get('dca-hero-gain').textContent  = '+'+fmtK(Math.round(gain));
  if(get('dca-hero-inv'))   get('dca-hero-inv').textContent   = fmtK(Math.round(invested));
  if(get('dca-hero-mult'))  get('dca-hero-mult').textContent  = '×'+(invested>0?(total/invested).toFixed(2):'1.00');
  const circle = get('dca-donut-circle');
  if(circle){ const c=2*Math.PI*36; circle.setAttribute('stroke-dasharray',Math.min(gainPct/100,1)*c*0.75+' '+c); }
  if(get('dca-donut-pct')) get('dca-donut-pct').textContent = gainPct+'%';
}
